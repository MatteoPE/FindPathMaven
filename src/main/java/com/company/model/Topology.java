...
public class Topology implements BFSConstant {
    ...
    public Set<Position> getSwitchedOffPositions() {

        return switchedOffNodes.stream().map(Node::getId).map((k) -> {
            int x = k/this.size;
            int y = k%this.size;
    ...
    public Set<Integer> getNeighborsID(int nodeID) {

        Node n = nodes.get(nodeID);
        return this.getNeighbors(n).get(1).stream()
                .map(Node::getId)
                .collect(Collectors.toSet());
